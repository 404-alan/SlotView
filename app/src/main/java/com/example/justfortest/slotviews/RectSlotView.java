package com.example.justfortest.slotviews;

import android.content.Context;
import android.graphics.Point;
import android.graphics.RectF;
import android.util.AttributeSet;

/**
 * Created on 12/19/20.
 *
 * @author Alan
 */
public class RectSlotView extends BorderImageView {
    float slotStartX = 137.1768f;
    float slotStartY = 136.7521f;
    float slotEndX = slotStartX + 633f;
    float slotEndY = slotStartY + 380f;

    public RectSlotView(Context context) {
        super(context);
    }

    public RectSlotView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public RectSlotView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override public RectF getSlotRect() {
        return new RectF(slotStartX, slotStartY, slotEndX, slotEndY);
    }

    @Override public Point getCenterPoint() {
        int centerX = (int) (slotStartX + slotEndX) /2;
        int centerY = (int) (slotStartY + slotEndY) / 2;
        return new Point(centerX,centerY);
    }

    @Override public String getOverlayName() {
        return "transparent.png";
    }
}
